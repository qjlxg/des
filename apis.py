...
class SSPanelSession(_ROSession):
    ...
    def register(self, email: str, password=None, email_code=None, invite_code=None, name_eq_email=None, reg_fmt=None, im_type=False, aff=None) -> str | None:
        ...
        # --- 配合你新增的探测逻辑，增加以下兼容性处理 ---
        # 探测时如果发现是 material 或 v2free，我们可以通过 self.base 判断
        is_v2free_style = 'material' in self.base or 'v2free' in self.base.lower()
        # 原有的 reg_fmt == 'B' 逻辑保留，同时增加对 v2free 风格的判定
        ...
        res = self.post(f'{self.auth_path}/register', {
            ...
            'passwd': password,
            'repasswd': password,
            email_code_k: email_code or '', # 使用动态确定的 key
